...
//   system("cp dftb_in.hsd-CG dftb_in.hsd");
   status=system("./dftb+>2");
//   printf("CG= %12.4f ",grep_energy());
//   system("cp dftb_in.hsd-ST dftb_in.hsd");
//   status=system("./dftb+>1");
//   printf("ST= %12.4f",grep_energy());
//   system("cp dftb_in.hsd-MD dftb_in.hsd");
//   status=system("./dftb+>1");
//   printf("MD= %12.4f\n",grep_energy());
    energy=grep_energy();
    fp3 =fopen("geo_end.gen","r"); read_gen(fp3,n1,n2,nc,xx,yy);fclose(fp3); 
 //  system("cp geo.gen geo.gen-2");
 printf("Energy= %12.4f\n",energy);
 return grep_energy();                             
 if(energy>0) exit(0);
   fclose(fp3);
...
//     for (ii=0;ii<13;ii++) printf("type  atom[%d]= %d %lf\n",ii,type_new[ii],p1copy[ii][0]);
     ...
     if(beta_population[i].ep-Ne_mate<population[min_index].ep)
       {
         printf(" Eold= %12.4lf  Enew= %12.4lf %d %d SUCCESSFULLY MATED !!! \n", population[min_index].ep,beta_population[i].ep,randa,randb);
       }
      else  
        {
          printf(" Eold= %12.4lf  Enew =%12.4f %d %d NOT          MATED !!!\n" ,population[min_index].ep,beta_population[i].ep,randa,randb);
          i=i-1;          
...
// FILE *fp_input = fopen("first_population","rb");
 FILE     *fp_input2 = fopen("ga_input2.4","r");
 int       NSTEP=0;
 int       step=0;
...
//initilization
 ...
 write_coord(fpoptim,ptnatm,runatm,cnatm,population[0].gen,population[0].cgen,population[0].ep);
 write_coord(fpoptim,ptnatm,runatm,cnatm,population[1].gen,population[0].cgen,population[0].ep);
 write_coord(fpoptim,ptnatm,runatm,cnatm,population[2].gen,population[0].cgen,population[0].ep);
 write_coord(fpoptim,ptnatm,runatm,cnatm,population[3].gen,population[0].cgen,population[0].ep);
// write_coord(fpoptim,ptnatm,runatm,cnatm,population[1].gen,population[1].cgen,population[1].ep);
 fflush(fpoptim);
 printf("argc=%d\n",argc);
 cal_pop_energy(POPSIZE,population,ptnatm,runatm,cnatm,cell,argc, argv);
// mutate_perm(ptnatm,runatm, Mu, POPSIZE, population);
